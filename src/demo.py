...
def generate_song_description(crypto, df):
    ...
    the beat, the instrument the style and the dynamic. You answer should be on the following format: ' A song ### song description'.
    
    The dataframe: {data}
    
    """
    prompt_template = PromptTemplate.from_template(prompt_template)
    prompt = prompt_template.format(crypto=crypto, data=df[-20:].to_string())
...
def main():
    ...
    if st.button("Create"):
        ...
        song_description = generate_song_description(crypto, df)

        st.subheader("Step 2: Convert the text into instructions for AudioCraft")
        st.info(song_description)

        create_audio(song_description)

        st.subheader("Step 3: Create the video and mix it with the created audio")
